...
from student import policy_iteration, value_iteration
from frozenlake_custom import FrozenLakeCustomEnv
import argparse
import random
import numpy as np
random.seed(1)
...
def main():
    ...
    for i in range(3):
        ...
        env = FrozenLakeCustomEnv(desc=env_map, is_slippery=True, render_mode='human' if render else 'rgb_array')

        policy, values = policy_iteration(env, ENV_SIZE, END_STATE, DIRECTIONS, OBSTACLES)
        # policy, values = value_iteration(env, ENV_SIZE, END_STATE, DIRECTIONS, OBSTACLES)
